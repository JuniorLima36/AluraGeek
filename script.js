document.addEventListener('DOMContentLoaded', () => {
  ...
  async function addProduct(event) {
    ...
    if (name.length < 3) {
      showNotification('O nome deve ter pelo menos 3 caracteres.');
      return;
    }
  
    const urlPattern = /^(https?:\/\/)?([\w\d-]+\.)+[\w\d-]+(\/[\w\d- .\/?%&=]*)?$/;
    if (imageUrl && !urlPattern.test(imageUrl)) {
      showNotification('A URL da imagem não é válida.');
      return;
    }
    ...
    if (imageUrl) {
      ...
      showNotification('Por favor, forneça uma imagem via URL ou upload!');
      return;
    }
    ...
    if (editingId) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          showNotification('Erro ao atualizar produto!');
        }
      } catch (error) {
